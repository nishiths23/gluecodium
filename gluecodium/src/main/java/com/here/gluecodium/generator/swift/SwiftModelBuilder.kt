...
class SwiftModelBuilder(
    ...
    private fun finishBuildingInterface(limeContainer: LimeInterface) {
        ...
        val isSwiftOnly = hasWeakProperties(limeContainer)
        val swiftClass = SwiftClass(
    ...
    companion object {
        ...
        fun hasWeakProperties(limeContainer: LimeContainerWithInheritance): Boolean {
            if (limeContainer.properties.any { it.attributes.have(SWIFT, WEAK) }) return true
            val parentContainer = limeContainer.parent?.type?.actualType as? LimeContainerWithInheritance
            return parentContainer != null && hasWeakProperties(parentContainer)
        }
    }
}
