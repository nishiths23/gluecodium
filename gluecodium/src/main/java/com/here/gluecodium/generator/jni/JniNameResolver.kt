...
internal class JniNameResolver(
    ...
    private fun resolveAccessorName(element: Any, rule: JavaNameRules.(LimeTypedElement) -> String): String? {
        ...
        return javaNameRules.rule(limeTypedElement) +
            if (limeTypedElement.attributes.have(CACHED)) "_1private" else ""
    }
    ...
    private fun resolveTypeName(limeType: LimeType) =
        when (val actualType = limeType.actualType) {
            ...
            is LimeMap -> "java/util/Map"
                ...
                val javaNames = resolveNestedNames(actualType).joinToString("$")
                (basePackages + actualType.path.head + javaNames).joinToString("/")
            }
        }

    private fun resolveFunctionName(limeFunction: LimeFunction) =
        (getParentElement(limeFunction) as? LimeLambda)?.attributes?.get(JAVA, FUNCTION_NAME)
            ?: javaNameRules.getName(limeFunction)
